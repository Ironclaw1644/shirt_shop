...
export default async function AddressesPage() {
  ...
  return (
    <div>
      ...
      <h1 className="heading-display mt-2 text-4xl">Shipping addresses</h1>

      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {(addrs ?? []).map((a) => (
          <div key={a.id} className="rounded-lg border border-ink/10 bg-card p-5 shadow-press">
            <p className="font-display font-semibold">{a.label ?? a.full_name ?? "Address"}</p>
        ...
        {(!addrs || addrs.length === 0) && (
          <div className="col-span-2 rounded-lg border border-dashed border-ink/20 bg-paper-warm p-10 text-center">
