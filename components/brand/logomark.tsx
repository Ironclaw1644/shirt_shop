...
export function Logomark({
  ...
  const outer = tone === "light" ? "#1A1A1A" : "#FAFAF7";
  const inner = tone === "light" ? "#FAFAF7" : "#1A1A1A";
  ...
  return (
    ...
    >
      <svg
        ...
        className={cn(
          "transform -rotate-6",
          animate && "transition-transform duration-300 group-hover:-rotate-12",
        )}
        style={{ width: size, height: size }}
      >
        <rect x="2" y="2" width="36" height="36" rx="8" fill={outer} />
        <rect x="6" y="6" width="28" height="28" rx="5" fill={inner} />
